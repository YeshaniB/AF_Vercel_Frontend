...
const CountryDetails = () => {
  ...
  useEffect(() => {
    const fetchCountryData = async () => {
      try {
        ...
        // Store recently viewed countries per user
        if (user && countryData) {
          const key = `viewedCountries_${user.username}`;
          const viewedHistory = localStorage.getItem(key) || '[]';
          let viewedCountries = JSON.parse(viewedHistory);

          // Remove if already exists
          viewedCountries = viewedCountries.filter(code => code !== countryCode);

          // Add current country code to front
          viewedCountries.unshift(countryCode);

          // Limit to 10
          viewedCountries = viewedCountries.slice(0, 10);

          localStorage.setItem(key, JSON.stringify(viewedCountries));
        }
      } catch (err) {
  ...
  if (error) {
    return (
      <div
        className="bg-red-100 border border-red-400 text-red-700 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400 px-4 py-3 rounded"
        role="alert"
      >
        <p>{error}</p>
      </div>
  ...
  if (!country) {
    return (
      <div
        className="bg-yellow-100 border border-yellow-400 text-yellow-700 dark:bg-yellow-900/20 dark:border-yellow-800 dark:text-yellow-400 px-4 py-3 rounded"
        role="alert"
      >
  ...
  // Format helpers
  const formatObjectToArray = (obj) => obj ? Object.values(obj) : [];

  const languages = formatObjectToArray(country.languages);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div>
          {country.flags?.svg ? (
            ...
            <div className="w-full h-48 bg-gray-300 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400">
              No flag available
            </div>
          )}
        </div>
